...
function Upload() {
  ...
  function getRank(score) {
    if (score < 10) return 'Bronze I'
    if (score < 20) return 'Bronze II'
    if (score < 30) return 'Bronze III'
    if (score < 40) return 'Silver I'
    if (score < 50) return 'Silver II'
    if (score < 60) return 'Silver III'
    if (score < 65) return 'Gold I'
    if (score < 70) return 'Gold II'
    if (score < 74) return 'Gold III'
    if (score < 78) return 'Platinum I'
    if (score < 82) return 'Platinum II'
    if (score < 85) return 'Platinum III'
    if (score < 88) return 'Diamond I'
    if (score < 91) return 'Diamond II'
    if (score < 94) return 'Diamond III'
    if (score < 96) return 'Elite I'
    if (score < 98) return 'Elite II'
    if (score < 99) return 'Elite III'
    return 'Mythic'
  }
  ...
  async function analyzeScores() {
    ...
    try {
      ...
      const routine = weakSkills.slice(0, 4).map(([skill]) => ({
        ...
        skill_target: skill.replace(/_/g, ' '),
        duration_mins: 7,
      ...
      const parsed = {
        ...
        weak_skills: weakSkills.map(([skill, score]) => ({
          ...
          pattern: `Your ${skill.replace(/_/g, ' ')} score of ${score.toFixed(0)} is below your average of ${(avgScore / 10).toFixed(0)} — this is a consistent gap not an isolated mistake.`,
      ...
      for (const [skill, score] of Object.entries(parsed.skill_scores)) {
        await supabase.from('skill_scores').insert({
          ...
          rank: getRank(score),
          failure_rate: parsed.weak_skills.find(w => w.skill.toLowerCase().includes(skill.replace(/_/g, ' ')))?.failure_rate || 0,
        })
      }
      ...
      {state === 'entry' && (
        <div>
          ...
          <div style={s.card}>
            ...
            {benchmarkScenarios.map((sc, i) => (
              <div key={sc.id} style={{ display: 'flex', alignItems: 'center', gap: '12px', padding: '12px 0', borderBottom: i < benchmarkScenarios.length - 1 ? '0.5px solid rgba(255,255,255,0.05)' : 'none' }}>
                <span style={{ fontSize: '11px', color: 'rgba(255,255,255,0.25)', minWidth: '20px' }}>{String(i + 1).padStart(2, '0')}</span>
                <div style={{ flex: 1 }}>
                  <div style={{ fontSize: '13px', fontWeight: '500', marginBottom: '2px' }}>{sc.scenario}</div>
                  <div style={{ fontSize: '11px', color: 'rgba(255,255,255,0.35)' }}>{sc.skill} · {sc.duration}</div>
                </div>
                <input
                  style={{ background: '#0e0e10', border: scores[sc.id] ? '0.5px solid rgba(127,119,221,0.4)' : '0.5px solid rgba(255,255,255,0.15)', borderRadius: '6px', padding: '8px 10px', color: '#f0f0f0', fontSize: '14px', outline: 'none', width: '90px', textAlign: 'center', fontFamily: 'inherit' }}
                  type="number"
                  placeholder="Score"
                  value={scores[sc.id] || ''}
                  onChange={e => updateScore(sc.id, e.target.value)}
                />
              </div>
            ))}
          </div>
        ...
        <div style={{ textAlign: 'center', padding: '60px 0' }}>
          ...
          <div style={{ background: '#141416', borderRadius: '10px', padding: '14px 18px', display: 'flex', alignItems: 'center', gap: '14px' }}>
            <div style={{ flex: 1, height: '4px', background: 'rgba(255,255,255,0.08)', borderRadius: '2px', overflow: 'hidden' }}>
              <div style={{ height: '100%', width: '60%', background: '#7F77DD', borderRadius: '2px' }}></div>
            </div>
